  ...
  if (!selected) {
    return (
      <div className="flex-1 min-h-0 lg:h-full flex items-center justify-center border-2 border-white/20 border-dashed rounded-t-lg lg:ml-4">
        <p className="text-white/30 text-xs sm:text-sm">{t('selectPrompt')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 min-h-0 lg:h-full flex flex-col border-2 border-white/40 rounded-t-lg lg:ml-4 overflow-hidden">
      ...
      <div className="w-full h-10 flex items-center justify-between px-3 sm:px-4 border-b border-b-white/25 shrink-0">
        <span className="text-white/60 text-xs sm:text-sm">{t('preview')}</span>
      ...
      <div className="relative w-full h-36 sm:h-44 lg:h-48 shrink-0 border-b border-b-white/20">
        <Image src={selected.image} alt={selected.alt} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
      </div>
      ...
      <div className="relative flex-1 min-h-0">
        <div ref={mainRef} onScroll={handleScroll} className="flex flex-col gap-2 p-3 sm:p-4 lg:p-5 pb-5 sm:pb-6 lg:pb-7 overflow-y-auto h-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {t(`focus.${selected.focus}`)}
            </span>
          </div>

          <p className="text-white/60 text-xs sm:text-sm text-justify leading-relaxed">
            {t(`items.${selected.translationKey}.description`)}
          </p>

          <div className="h-px bg-white/10" />

          <div className="flex flex-col gap-2">
            <span className="text-white/40 text-xs uppercase tracking-widest">{t('technologies')}</span>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-white/40 text-xs uppercase tracking-widest">{t('links')}</span>
